...
std::pair<int, int> showLiveFeedback2(const std::string &expected,
  ...
  for (size_t i = 0; i < expected_words.size(); ++i) {
    ...
    if (i < typed_words.size()) {
      ...
      for (size_t j = 0; j < expected_word.length(); ++j) {
        bool is_cursor_pos = typed_char_index + j == typed.length() && !trailing_space;
        if (is_cursor_pos) std::cout << UNDERLINE;

        if (j < typed_word.length()) {
          if (typed_word[j] == expected_word[j]) {
            std::cout << GREEN << expected_word[j] << RESET;
            total_correct++;
          } else {
            std::cout << RED << expected_word[j] << RESET;
            total_incorrect++;
          }
        } else {
          std::cout << expected_word[j];
        }
        if (is_cursor_pos) std::cout << NO_UNDERLINE;
      }
      ...
      // This is an untyped word. Place cursor at the beginning if it belongs here.
      bool is_cursor_pos = typed_char_index == typed.length() && !trailing_space;
      if (is_cursor_pos) std::cout << UNDERLINE;
      std::cout << expected_word;
      if (is_cursor_pos) std::cout << NO_UNDERLINE;
    }

    // Correctly advance the character counter and render the space
    if (i < expected_words.size() - 1) {
        // **THE FIX IS HERE**: Use the typed word's length for the counter
        if (i < typed_words.size()) {
        ...
        bool is_cursor_pos = (typed_char_index - 1 == typed.length()) ||
  ...
  if (typed.length() > 0 && typed.length() == typed_char_index + typed_words.back().length()) {
      std::cout << UNDERLINE << " " << NO_UNDERLINE;
  }
