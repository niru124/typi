...
int main() {
  ...
  while (true) {
    ...
    if (c == 127 || c == 8) { // backspace
      ...
      if (c == ' ') {
        if (!current_word.empty() && y.size() < store.size()) {
          // Fill the rest of the current word with expected characters
          string &expected_word = store[y.size()];
          size_t current_word_len = current_word.length();
          for (size_t i = current_word_len; i < expected_word.length(); ++i) {
            typed += expected_word[i];
          }
          typed += ' '; // Add the space that was pressed
        ...
        } else if (current_word.empty() && y.size() < store.size()) {
          // If current_word is empty, but a space is pressed, it means the user
          // is skipping a word.
          string &expected_word = store[y.size()];
          skipped_chars_total += expected_word.length(); // Add length of skipped word
          typed += expected_word; // Append the expected word to typed
          typed += ' ';           // Add the space that was pressed
          // Record WPM for the skipped word as 0
          y.push_back(0.0);
        } else {
          typed += ' '; // Just add space if no word to process or already processed
        }
        current_word.clear();
  ...
  int grossWPM = (((res.first - skipped_chars_total) / 5.0) / total_minutes);
